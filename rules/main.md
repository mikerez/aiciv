...
- `MAIN-AI-012`: Every authenticated browser may contribute work to the global AI during its human turn. PHP leases disjoint batches of unassigned movable AI objects using weighted service debt. Mature Settlers with a validated destination receive one atomic movement opportunity each turn; mature Settlers without a mission retain an eight-turn service interval, so blocked units cannot monopolize the server while Workers and military forces continue accumulating service debt.
...
- `MAIN-AI-017`: A shared AI Settler persists only its intended world destination in authoritative unit properties. Every contributor lease rebuilds a fresh local route from the current server position, and accepts it only when the route actually ends on that destination; reaching or failing to reach the destination clears the mission. PHP never persists the route itself.
