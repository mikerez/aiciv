...
| Longbow | land | 5 | 3 | 1 | 3 | Archery | 55 | Copper or Iron |
...
- `PREHISTORY-AUTO-019`: Barbarian Cities retain their strongest co-located military unit as a fortified garrison. Additional military units and military units outside Cities keep model-selected attacks or movement; when otherwise idle, half search toward nearby fog and half patrol for visible wartime enemies or around their current area. These roaming forces persist in Patrol mode and receive more frequent AI scheduling than fortified defenders. Global AI scheduling compares weighted turns-since-service across development and military work, so recently serviced Settlers or Workers cannot indefinitely block an overdue force.
- `PREHISTORY-AUTO-020`: A Barbarian Settler founds a City only on grass or grass-with-water terrain, and established Cities must be at least seven hex steps apart. A mature Settler keeps searching rather than using its age limit to found on hills, rocks, forest, desert, snow, or water.
- `PREHISTORY-AUTO-021`: Every new Barbarian City receives up to two nearby automated support Workers. A one-time migration supplies existing Barbarian Cities that have fewer than two Workers within five Tiles. An idle Worker near more than two Workers rebalances toward a visible City with less support before selecting another local improvement.
- `PREHISTORY-AUTO-022`: A Barbarian City with enough stored food to grow receives elevated AI service even while producing. Explorers receive ordinary-force scheduling weight so exact-Tile resource discovery continues across the empire.
...
- `PREHISTORY-ROAD-003`: Road building takes six client turns before its authoritative build request is submitted.
