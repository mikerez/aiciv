...
if (lines[0].color !== '#00b83e' || lines[0].width !== 38) throw new Error('75% HP must be green at half scale');
if (lines[1].color !== '#1687ff' || lines[1].width !== 25) throw new Error('50% XP must be blue at half scale');
if (lines[2].color !== '#ffd400' || lines[2].width !== 25) throw new Error('50% HP must be yellow at half scale');
if (lines[3].color !== '#9b36d6' || lines[3].width !== 30) throw new Error('60% XP must be purple at half scale');
if (lines[4].color !== '#e02020' || lines[4].width !== 5) throw new Error('10% HP must be red at half scale');
if (lines[5].color !== '#9b36d6' || lines[5].width !== 50) throw new Error('full XP must be purple at half scale');
if (lines[0].x !== 175 || lines[1].x !== 175) throw new Error('HP and XP segments must share the centered bar left edge');
if (lines[5].width !== 50) throw new Error('a 100% status line must fill the shared half-tile width');
